...
public abstract class Zipper<A, TA extends Traversable<A, ?>>
    implements CoProduct2<TA, Tuple2<A, Function<Maybe<A>, Zipper<A, TA>>>,
                          Zipper<A, TA>> {

  private Zipper() {}

  public static <A, T extends Traversable, TA extends Traversable<A, T>>
      Zipper<A, TA> zipper(TA ta) {
    return reset(
        ta.<A, Cont<Zipper<A, TA>, ?>, TA, Cont<Zipper<A, TA>, A>,
            Cont<Zipper<A, TA>,
                 TA>>traverse(a
                              -> shift(k
                                       -> new Cont<>(
                                           ...
                                           -> g.apply(hole(
                                               a,
                                               maybeA
                                               -> k.apply(maybeA.orElse(a)))))),
                              ta_ -> new Cont<>(g -> g.apply(ta_)))
            .flatMap(trav -> new Cont<>(g -> g.apply(top(trav)))));
  }
  ...
  public static <A, TA extends Traversable<A, ?>> Zipper<A, TA>
  hole(A a, Function<? super Maybe<A>, Zipper<A, TA>> nextFn) {
    return new Hole<>(a, fn1(nextFn));
  }

  public static void main(String[] args) {
    Zipper<Integer, LambdaIterable<Integer>> zipper =
        zipper(LambdaIterable.wrap(take(100000, iterate(x -> x + 1, 0))));
  ...
  private static final class Top<A, TA extends Traversable<A, ?>>
      extends Zipper<A, TA> {
    private final TA ta;

    private Top(TA ta) { this.ta = ta; }

    @Override
    public <R>
        R match(Function<? super TA, ? extends R> aFn,
                Function<? super Tuple2<A, Function<Maybe<A>, Zipper<A, TA>>>, ?
                             extends R> bFn) {
      return aFn.apply(ta);
    }
  }

  private static final class Hole<A, TA extends Traversable<A, ?>>
      extends Zipper<A, TA> {
    private final A a;
